...
const EngineeringScope = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 1. أنيميشن العنوان (يعمل على الكل)
      gsap.from(".main-title-part", {
        y: 40,
        opacity: 0,
        duration: 1,
      ...
      // 2. إعدادات خاصة للشاشات الكبيرة (Desktop)
      mm.add("(min-width: 768px)", () => {
        // أنيميشن العمود الفقري
        gsap.from(".central-spine", {
          height: 0,
          scrollTrigger: {
            trigger: triggerRef.current,
      ...
      // 3. إعدادات خاصة للشاشات الصغيرة (Mobile)
      ...
      // 4. أنيميشن نص الالتزام (يعمل على الكل)
      gsap.from(".focus-statement", {
        opacity: 0,
        y: 20,
        duration: 1.2,
  ...
  return (
    ...
    >
      {/* Background Grid - Adjusted for better responsiveness */}
      <div
        className="bg-grid absolute inset-0 opacity-5 pointer-events-none"
        style={{
          backgroundImage: `linear-gradient(to right, #C5A363 1px, transparent 1px), linear-gradient(to bottom, #C5A363 1px, transparent 1px)`,
      ...
      <div ref={triggerRef} className="max-w-7xl mx-auto relative">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center mb-12 md:mb-20">
          ...
          <div className="main-title-part relative p-1 inline-block w-full max-w-3xl">
            ...
            <div className="bg-white/[0.02] backdrop-blur-sm border border-white/5 px-4 py-6 md:px-8 md:py-8">
              <p className="text-base md:text-xl font-light leading-relaxed text-lightColor opacity-80 normal-case italic">
                Mosaic Property Management focuses{" "}
          ...
          {sectors.map((sector, i) => (
            ...
            >
              ...
              <div className="flex-1 w-full bg-mainColor/80 border border-white/10 backdrop-blur-md p-6 md:p-10 relative group hover:border-mainGold/40 transition-all duration-500 overflow-hidden rounded-sm">
                ...
                <div className="absolute -top-2 -left-2 md:-top-4 md:-left-4 text-5xl md:text-7xl font-black text-white/5 group-hover:text-mainGold/10 transition-colors italic leading-none select-none">
                  {sector.id}
                </div>
              ...
              {/* Empty Space for Spine Alignment */}
              <div className="flex-1 hidden md:block" />
            </div>
          ))}
        </div>
        ...
        {/* Exclusion/Focus Statement */}
        <div className="focus-statement relative z-10 mx-auto mt-8 md:mt-10 text-center px-4">
          
          <p className="text-[9px] md:text-xs tracking-[0.2em] md:tracking-[0.4em] text-mainGold/60 max-w-2xl mx-auto uppercase mb-4 md:mb-6 font-bold leading-loose">
            We do not engage in residential sales, condominium sales, or
            condominium management.
          </p>
